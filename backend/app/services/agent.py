from app.config import EARTHDIAL_MODEL, OMNIROUTE_POLISH
...
def _is_spatial_query(q: str) -> bool:
    return any(x in q for x in (
        "where", "locate", "location", "highlight", "mark", "show me", "find", "detect",
        "bounding", "bbox", "building", "buildings", "road", "roads", "water", "ship", "ships",
        "field", "fields", "vegetation patch", "region", "area of", "object", "objects"
    ))


def select_task(query, task_hint, metas):
    forced = {"vqa":"vqa", "caption":"caption", "grounding":"grounding", "change":"change_detection", "optical_sar":"optical_sar", "metadata":"metadata"}
    hint = _text(task_hint or "auto")
    if hint in forced:
        return forced[hint]
    q = _text(query)
    if len(metas) >= 2 and any(x in q for x in ("change", "changed", "difference", "compare", "before and after", "temporal")):
        return "change_detection"
    modalities = [geospatial.estimate_modality(m, m.get("filename", "")) for m in metas]
    if len(metas) >= 2 and (("optical" in modalities and "sar" in modalities) or any(x in q for x in ("sar", "radar", "backscatter", "optical + sar", "optical and sar"))):
        return "optical_sar"
    if _is_spatial_query(q):
        return "grounding"
...
def _select_with_router(query, task_hint, metas):
    ...
    if _is_spatial_query(_text(query)):
        return deterministic, "deterministic_spatial", "Spatial query routed to grounding before optional OmniRoute selection."
    modalities = [geospatial.estimate_modality(m, m.get("filename", "")) for m in metas]
...
def run(files_data, metas, query, task_hint="auto"):
    ...
    params = {"num_images": len(metas), "modalities": [geospatial.estimate_modality(m, m.get("filename", "")) for m in metas]}
    if task == "metadata":
    ...
    if OMNIROUTE_POLISH and router == "omniroute":
        answer = omniroute_client.polish_answer(query, answer, task)
    trace = {"selected_task": task, "tools_used": [tool], "model_used": result.get("model_used", EARTHDIAL_MODEL), "parameters": {**params, **result.get("parameters", {})}, "router": router, "router_reason": router_reason}
    return {"answer": answer, "geojson": result["geojson"], "confidence": max(0.0, min(1.0, float(result.get("confidence", 0.5)))), "execution_trace": trace, "model_used": trace["model_used"]}
