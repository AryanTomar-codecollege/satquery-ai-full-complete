...
def choose_tool(query: str, num_images: int, modalities: List[str], task_hint: str = "auto") -> Dict:
    ...
    system = (
        "You are the query router for SatQuery AI. "
        "Choose exactly one specialist tool. Return only valid JSON with keys "
        "\"tool\" and \"reason\". Do not analyze image pixels. "
        ...
        "Use optical_sar for optical+SAR paired requests. "
        "Use grounding for where/locate/highlight requests. "
        "Use metadata for CRS/dimensions/bands/dtype/resolution questions. "
        "Use caption for describe/scene-description requests. "
        "Use vqa for other image questions."
    )
    user = (
        f"query={query}\n"
...
def polish_answer(query: str, answer: str, selected_task: str) -> str:
    if not configured() or not answer:
        return answer
    system = (
        "Rewrite the supplied remote-sensing answer for clarity. "
        "Do not add facts, numbers, coordinates, objects, locations, or certainty. "
        "Preserve uncertainty. Return only the rewritten answer."
    )
    user = (
    ...
    try:
        ...
        response.raise_for_status()
        return str(response.json()["choices"][0]["message"]["content"]).strip() or answer
    except Exception:
        return answer
