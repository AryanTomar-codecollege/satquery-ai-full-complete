...
def run(file_tuple, query: str):
    ...
    return {"answer": answer, "geojson": geojson, "confidence": estimate_confidence("vqa", answer, spatial_features=len(geojson["features"])), "model_used": result.get("model", "EarthDial_4B_MS"), "parameters": {"tag": result.get("tag", "[vqa]"), "boxes_parsed": len(geojson["features"])} }
